<template>
	...
	<ModalWindow v-model="modal" :title="$t('create-new-aspect')">
		<EditAspect
			mode="new"
...
export default defineComponent({
	...
	methods: {
		...
		remove(id: number) {
			this.updateAspects(
				this.characters[this.current].aspects.filter(
					(e: Aspect, i: number) => i !== id
				)
			);
		},
		add(aspect: Aspect) {
			this.updateAspects([
				...this.characters[this.current].aspects,
				aspect,
			]);
		},
	},
