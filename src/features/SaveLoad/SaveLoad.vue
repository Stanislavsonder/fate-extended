<template>
	...
	<ModalWindow v-model="modal" :title="$t('load-characters-file')">
...
export default defineComponent({
	...
	methods: {
		save() {
			const file = new Blob(
				[JSON.stringify(this.characters[this.current])],
				{
					type: "text/plain",
				}
			);
			...
			link.download =
				(this.characters[this.current].name ||
					this.$t("unnamed-character")) + ".fate";
			link.click();
